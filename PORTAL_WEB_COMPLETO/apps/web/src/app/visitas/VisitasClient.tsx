...
import { fetchVisits, upsertVisit, deleteVisit, fetchUsers } from '@/lib/supabase';
...
export function VisitasClient({ initialNew }: { initialNew?: boolean }) {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    const [v, users] = await Promise.all([
      fetchVisits({ limit: '2000' }),
      fetchUsers(),
    ]);
    setVisits(v);
    setTechList(users);
  ...
  function openEdit(v: Visit) {
    ...
    setForm({
      ...
      notes: v.notes,
      visit_date: v.visit_date ? v.visit_date.substring(0, 10) : '',
    });
    setFormError('');
    setModalOpen(true);
  }
  ...
  async function handleSave() {
    if (!form.technician_name || !form.client_name || !form.visit_date) {
                ...
                {pageData.length === 0 ? (
                  ...
                  pageData.map((v, i) => (
                    <tr key={v.id || i} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 text-gray-700 whitespace-nowrap">{fmtDate(v.visit_date)}</td>
      ...
      >
        {detailTarget && (
          <div className="space-y-3 text-sm">
            {([
              ['Data', fmtDate(detailTarget.visit_date)],
