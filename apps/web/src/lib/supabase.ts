import type { ClientRecord, Visit, MonthlySale, ScheduleItem, TechUser } from '@/types/portal';
...
function extractClientFromNotes(notes: string): string {
  if (!notes) return '';
...
export async function fetchClients(params: Record<string, string> = {}): Promise<ClientRecord[]> {
  const base: Record<string, string> = { select: '*' };
  const merged = { ...base, ...params };
  const parts: string[] = [];
  for (const [k, v] of Object.entries(merged)) {
    parts.push(`${k}=${v}`);
  ...
  const endpoints = ['/Client', '/Clients', '/cliente', '/clientes'];
  for (const endpoint of endpoints) {
    const res = await supabaseFetch<Record<string, unknown>[]>(`${endpoint}?${parts.join('&')}`);
    if (res.ok && Array.isArray(res.data)) {
      return res.data.map((item) => ({
        id: String(item.id || item.client_id || crypto.randomUUID?.() || `${Date.now()}`),
        name: String(item.name || item.client_name || item.nome || ''),
        technician_name: String(item.technician_name || item.techName || item.technician || ''),
        area: String(item.area || item.areas || ''),
      }));
    }
  }
...
export async function fetchSchedule(params: Record<string, string> = {}): Promise<ScheduleItem[]> {
  ...
  const endpoints = ['/Schedule', '/Agenda', '/Agendamento'];
  for (const endpoint of endpoints) {
    const res = await supabaseFetch<Record<string, unknown>[]>(`${endpoint}?${parts.join('&')}`);
    if (res.ok && Array.isArray(res.data)) {
      return res.data.map((item) => ({
