...
async function runMigrations() {
  try {
    // Cria a tabela se nao existir (sem a coluna technicianName ainda)
    ...
    // Adiciona coluna technicianName se ainda nao existir
    await prisma.$executeRawUnsafe(`
      ALTER TABLE "monthly_sales"
      ADD COLUMN IF NOT EXISTS "technicianName" TEXT NOT NULL DEFAULT '';
    `);

    // Remove indice unico antigo (month unico) se existir
    await prisma.$executeRawUnsafe(`
      DROP INDEX IF EXISTS "monthly_sales_month_key";
    `);

    // Cria indice unico composto (month + technicianName) se nao existir
    await prisma.$executeRawUnsafe(`
