...
// Sincroniza visitas: envia TODAS as visitas locais para Supabase e recebe registros do servidor.
// CORRECAO: Forca re-sincronizacao de todas as visitas, nao apenas as sem server_id
async function syncVisits() {
  if (!hasApi()) return { ok: false, error: 'API nao configurada' };
  const visits = await load(KEY.VISITS);

  // Envia TODAS as visitas locais (forca re-sincronizacao)
  const pending = visits; // CORRECAO: envia todas, nao apenas sem server_id
  let sent = 0;
  const errors = [];
  for (const v of pending) {
    const payload = {
      ...
      animaisAcasalados: v.herdSize || v.animaisAcasalados || 0,
    };
    // NAO enviamos 'id' — deixamos o Supabase gerar automaticamente
    ...
    if (res.ok) {
      ...
      const errMsg = (res.error || `HTTP erro`).substring(0, 60);
      errors.push(`${v.propertyName || v.clientName || 'Visita sem nome'}: ${errMsg}`);
    }
  }
  ...
  for (const sv of serverVisits) {
    ...
    if (!svLocalId && !serverOnlyIds.has(svServerId)) {
      visits.push({
        ...
        area:           String(sv.areas || sv.area || '').trim().padStart(3, '0'),
        notes:          sv.notes        || '',
  ...
  if (hasPending && sent === 0) {
    return { ok: false, error: `Falha ao enviar ${pending.length} visita(s).\n` + errors.slice(0, 3).join('\n') };
  }
  return { ok: true, synced: sent, received: serverVisits.length, errors: errors.length > 0 ? errors : undefined };
}
            ...
            <View key={item.id} style={s.listCard}>
              <View style={s.techCardRow}>
                <Pressable
                  ...
                  {item.technicianName ? <Text style={s.muted}>Tecnico: {item.technicianName}</Text> : null}
...
function VisitsScreen({ go, onBack, session }) {
  ...
  const [selectedMonth,   setSelectedMonth]   = useState(() => {
    // TODOS os usuarios iniciam no mes atual (gestor e tecnico)
    const now = new Date();
  ...
  async function handleDelete(id) {
    ...
    // Remove do estado (tanto allItems quanto items)
    setAllItems(prev => prev.filter(v => v.id !== id));
  }
  ...
  async function handleSave() {
    ...
    await save(KEY.VISITS, [
      { id: localId, localId,
        ...
        clientName: scheduleData?.clientName || form.clientName || form.propertyName.trim(), // CORRECAO: preenche clientName
    ...
    // Tenta enviar para a API em background e informa o resultado
    try {
      const syncResult = await syncVisits();
      if (!syncResult.ok) {
        Alert.alert('Aviso', 'Visita salva localmente, mas nao foi enviada para o servidor.\n\nToque "Sincronizar agora" na tela inicial para tentar novamente.', [{ text: 'OK', onPress: onSaved }]);
        setBusy(false);
        return;
      }
    } catch (e) {
      console.warn('syncVisits error:', e);
    }
    setBusy(false);
    Alert.alert('Visita salva!', 'Registro salvo e sincronizado.', [{ text: 'OK', onPress: onSaved }]);
  }
